...
class InstituteRepository implements InstituteRepositoryImpl {
  ...
  Future<WhatsAppSettings?> getWhatsAppSettings() async {
    final response = await _apiClient.get(ApiConstants.instituteWhatsAppSettings);
    if (response.status.hasError) {
      if (response.statusCode == 404) return null;
      throw Exception('Failed to fetch WhatsApp settings: ${response.statusText}');
    }
  ...
  @override
  Future<WhatsAppSettings> saveWhatsAppSettings(Map<String, dynamic> data) async {
    final response = await _apiClient.post(
      ApiConstants.instituteWhatsAppSettings,
      data,
    );
    if (response.status.hasError) {
      throw Exception('Failed to save WhatsApp settings: ${response.statusText}');
    }
    return WhatsAppSettings.fromJson(response.body['data']);
  }

  @override
  Future<WhatsAppSettings> updateWhatsAppSettings(Map<String, dynamic> data) async {
    final response = await _apiClient.put(
      ApiConstants.instituteWhatsAppSettings,
      data,
    );
    if (response.status.hasError) {
      throw Exception('Failed to update WhatsApp settings: ${response.statusText}');
    }
    return WhatsAppSettings.fromJson(response.body['data']);
  }
  ...
  Future<FeeRecord> createFee(Map<String, dynamic> data) async {
    ...
    if (response.status.hasError) {
      final message = response.body?['message'] ?? 'Failed to create fee record';
      throw Exception(message);
    }
  ...
  Future<List<int>> exportFees() async {
    ...
    await for (final chunk in response.bodyBytes!) {
